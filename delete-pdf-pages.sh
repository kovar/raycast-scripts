...
# @raycast.title Delete PDF Pages
# @raycast.mode silent
# @raycast.icon 🗑️
# @raycast.description Delete a page or range from selected PDF and save as new file
...
while IFS= read -r file; do
  ...
  if [ "$start" -eq 1 ] && [ "$end" -eq "$total" ]; then
    echo "Cannot delete all pages from $(basename "$file")"
    continue
  fi

  keep=""
  [ "$start" -gt 1 ] && keep="1-$((start - 1))"
  if [ "$end" -lt "$total" ]; then
    [ -n "$keep" ] && keep="$keep,$((end + 1))-$total" || keep="$((end + 1))-$total"
  fi

  dir=$(dirname "$file")
  base=$(basename "${file%.*}")
  output="$dir/${base}_del${range}.pdf"
  counter=1
  while [ -e "$output" ]; do
    output="$dir/${base}_del${range}_${counter}.pdf"
    (( counter++ ))
  done

  qpdf "$file" --pages "$file" "$keep" -- "$output"
done <<< "$files"
